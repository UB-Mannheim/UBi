...
async def process_single_file_async(llm: ChatOpenAI, file_path, output_path, prompt):
    """
...
def process_markdown_files_with_llm(
    ...
    async def process_files_async():
        ...
        completed = 0
        for coro in tqdm(
            asyncio.as_completed(tasks), total=len(tasks), desc="LLM Processing"
        ):
            result = await coro
            if result:
                completed += 1
...
def process_direktion(data_path: Path, verbose: bool = False):
    ...
    try:
        ...
        profile_match = re.search(
            r"^Direktorin der Universitätsbibliothek\s*$", md_data, re.MULTILINE
        )
        if profile_match:
...
def process_shibboleth(data_path: Path, verbose: bool = False):
    ...
    parent_files = list(
        data_path.glob("*medien_hinweise-zu-e-books-e-journals-und-datenbanken.md")
    )
    if not parent_files:
        print(
...
def additional_post_processing(data_dir: str = str(DATA_DIR), verbose: bool = False):
    """
