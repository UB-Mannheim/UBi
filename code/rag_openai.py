...
async def async_upload_files_to_vectorstore(
    client: OpenAI, vectorstore_id: str, vectorstore_filenames: dict, md_files: list
):
    """
    ...
    try:
        if not OPENAI_VECTORSTORE_ID:
            ...
            vectorstore_filenames = asyncio.run(
                get_vectorstore_fileids_and_metadata(client, str(OPENAI_VECTORSTORE_ID))
            )
            ...
            reupload_performed, vectorstore_filenames = asyncio.run(
                check_and_reupload_if_attributes_empty(
                    client, str(OPENAI_VECTORSTORE_ID), DATA_DIR, vectorstore_filenames
                )
            )
